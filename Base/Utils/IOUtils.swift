...
open class IOUtils {
  ...
  public static func tm_fileSizeAtPath(_ filePath:String) -> UInt64 {
    ...
    /// 单个文件的修改时间
    /// - Parameter filePath: 文件路径
    public static func tm_fileTimeAtPath(_ filePath:String) -> Int {
        let manager = FileManager.default
        if manager.fileExists(atPath: filePath) {
            do {
                let attr = try FileManager.default.attributesOfItem(atPath: filePath)
    ...
    ///   - data: ,data 数据
    ///   - fileName: 文件名称
    public static func tm_dataWriteToFile(_ data:Data,_ fileName:String){
    ...
    public static func tm_dataWriteToFile(_ fileName:String) -> Any? {
        let filePath = kPATH_OF_DOCUMENT + "/" + fileName
        do {
    ...
    public static func tm_dataCacheFilePath(_ type:String) -> String {
    ...
    public static func tm_folderRemoveAtPath(_ folderPath:String) -> Bool {
        let fileManager = FileManager.default
